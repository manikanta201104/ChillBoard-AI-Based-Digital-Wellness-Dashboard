...
let totalTime = 0;
let tabTimers = {}; // Track start time per tab
let tabUsage = [];
let lastSync = 0;
const SYNC_INTERVAL_MS = 5 * 60 * 1000; // 5 minutes in ms
  ...
  chrome.storage.local.get(['totalTime', 'tabUsage'], (result) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs.length > 0 && tabs[0].id && tabs[0].url) {
        ...
        tabTimers[tabId] = { startTime: Date.now(), url: new URL(tabs[0].url).hostname.replace('www.', '') };
        console.log(getTimestamp(), 'Set initial active tab:', { tabId, url: tabTimers[tabId].url });
      } else {
        console.log(getTimestamp(), 'No active tab found on startup');
      }
      checkFocus();
    });
  });
...
chrome.tabs.onActivated.addListener((activeInfo) => {
  if (isTracking) {
    ...
    console.log(getTimestamp(), 'Tab activated but not tracking:', { tabId: activeInfo.tabId });
