...
const refreshAccessToken = async (userId, refreshToken) => {
  ...
  try {
    ...
    if (err.body && err.body.error === "invalid_grant") {
      ...
      throw new Error(
        "Refresh token invalid, please re-authenticate with Spotify",
      );
    }
...
router.get("/callback", async (req, res) => {
  ...
  if (error) {
    ...
    return res
      .status(400)
      .json({ message: "Spotify authorization failed", error });
  }

  const userId = Buffer.from(state, "base64").toString();

  try {
    // Handle admin users specially since they're not in User model
    if (userId === "admin") {
      logger.info(
        "Admin user Spotify callback - tokens stored in session/memory",
        { userId },
      );

      // For admin, we could store tokens in a separate collection or memory
      // For now, just log and redirect since admin doesn't need Spotify integration
      logger.info("Admin Spotify authorization completed", { userId });
      return res.redirect("https://www.chillboard.in/dashboard");
    }
    ...
    logger.error("Error in Spotify callback", {
      error: err.message,
      stack: err.stack,
    });
    if (err.body && err.body.error === "invalid_grant") {
      return res
        .status(400)
        .json({ message: "Invalid authorization code", error: err.message });
    }
...
router.get("/playlist", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.spotifyToken || !user.spotifyToken.accessToken) {
      return res
        .status(401)
        .json({ message: "Spotify token not found, please re-authenticate" });
    }

    const { accessToken, refreshToken, expiresIn, obtainedAt } =
      user.spotifyToken;
    const now = Date.now();
    ...
    if (now >= tokenExpiry - 300000) {
      logger.info("Token nearing expiry or expired, refreshing", {
        userId,
        tokenExpiry,
      });
      try {
        ...
        if (refreshErr.message.includes("re-authenticate")) {
          return res
            ...
            .json({
              message:
                "Invalid refresh token, please re-authenticate with Spotify",
    ...
    if (!skip) {
      ...
      if (cachedPlaylist) {
        logger.info("Returning cached playlist", {
          userId,
          playlistId: cachedPlaylist.spotifyPlaylistId,
        });
        return res
          .status(200)
          .json({
            spotifyPlaylistId: cachedPlaylist.spotifyPlaylistId,
    ...
    if (skip || !cachedPlaylist) {
      const category =
        moodCategoryMap[mood.toLowerCase()] || moodCategoryMap.default;

      let playlists;
      try {
        playlists = await spotifyApi.searchPlaylists(`category:${category}`, {
          limit: 10,
        });
        logger.info("Spotify API response", {
          items:
            playlists.body.playlists?.items?.map((p) => ({
              id: p?.id,
              name: p?.name,
            })) || [],
        });
        ...
        if (searchErr.statusCode === 401) {
          ...
          logger.error("Spotify server error, retrying after delay", {
            userId,
          });
      ...
      if (
        ...
        playlists.body.playlists.items.length === 0
      ) {
        return res
          .status(404)
      ...
      const availablePlaylists = playlists.body.playlists.items.filter(
        (p) =>
          p &&
          p.id &&
          (!cachedPlaylist || p.id !== cachedPlaylist?.spotifyPlaylistId),
      );
      if (availablePlaylists.length === 0 && !skip) {
        logger.warn("No new playlists available, forcing new search");
        playlists = await spotifyApi.searchPlaylists(
          `category:${category} calm`,
          { limit: 10 },
        );
        availablePlaylists = playlists.body.playlists.items.filter(
          (p) =>
            p &&
            p.id &&
            (!cachedPlaylist || p.id !== cachedPlaylist?.spotifyPlaylistId),
      ...
      if (availablePlaylists.length === 0) {
        return res
          .status(404)
          .json({ message: "No valid playlists available" });
      }
    ...
    if (err.message.includes("re-authenticate")) {
      return res
        .status(401)
        .json({
          message: "Invalid refresh token, please re-authenticate with Spotify",
          redirect: "/spotify/login",
        });
    }
    res.status(500).json({ message: "Server error", error: err.message });
...
router.post("/play", authMiddleware, async (req, res) => {
  ...
  if (!device_id || !playlist_id) {
    return res
      .status(400)
      .json({ message: "Missing device_id or playlist_id" });
  }
  ...
  try {
    ...
    try {
      ...
      logger.error("Invalid playlist", {
        playlist_id,
        error: playlistErr.message,
      });
      return res
        .status(404)
        .json({ message: "Invalid or inaccessible playlist" });
    }
    ...
    try {
      ...
      if (transferErr.statusCode === 403 || transferErr.statusCode === 404) {
        logger.warn("Transfer failed", {
          userId,
          device_id,
          reason: transferErr.body?.error?.reason || "UNKNOWN",
        });
        return res.status(transferErr.statusCode).json({
          message:
            "Failed to transfer playback. Ensure device is active and open Spotify.",
    ...
    if (err.statusCode === 401) {
      return res
        .status(401)
        .json({
          message: "Permissions missing or token invalid. Re-authenticate.",
        });
...
router.patch("/playlist/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!playlist) {
      logger.warn("Playlist not found for update", {
        spotifyPlaylistId: id,
        userId: req.user.userId,
      });
    ...
    if (playlist.saved && saved === true) {
      logger.info("Playlist already saved", {
        spotifyPlaylistId: id,
        userId: req.user.userId,
      });
      return res
        .status(200)
        .json({ message: "Playlist already saved", playlist });
    }
...
router.delete("/unlink", authMiddleware, async (req, res) => {
  try {
    ...
    logger.error("Error unlinking Spotify account", {
      error: error.message,
      stack: error.stack,
    });
