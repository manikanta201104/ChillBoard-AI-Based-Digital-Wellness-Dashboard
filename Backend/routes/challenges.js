...
router.post("/progress", authMiddleware, async (req, res) => {
  ...
  try {
      ...
      now >
        new Date(
          challenge.startDate.getTime() +
            challenge.duration * 24 * 60 * 60 * 1000,
        )
    ) {
    ...
    const anchorDate = new Date(
      Math.max(
        challenge.startDate.getTime(),
        participant.joinedAt || challenge.startDate.getTime(),
      ),
    );
    ...
    const lastUpdateDay = new Date(
      new Date(participant.lastUpdate).setHours(0, 0, 0, 0),
    );
    if (
      !manualTrigger &&
      lastUpdateDay.getTime() === startOfCurrentDay.getTime()
    ) {
      logger.info("Progress update skipped, already updated today", {
        challengeId,
        userId,
    ...
    let prevDoc = await ScreenTime.findOne({
      userId,
      date: { $lt: startOfCurrentDay, $gte: anchorYmd },
    }).sort({ date: -1 });
    if (!prevDoc) {
      prevDoc = await ScreenTime.findOne({
        userId,
        date: { $lt: startOfCurrentDay },
      }).sort({ date: -1 });
    }
    const todayDoc = await ScreenTime.findOne({
      userId,
    ...
    if (!prevDoc || !todayDoc) {
      logger.info(
        "Insufficient data for update (need current day and a previous tracked day since anchor)",
        { challengeId, userId, hasPrev: !!prevDoc, hasCurrent: !!todayDoc },
      );
    ...
    if ((prevDoc.totalTime || 0) === 0 || (todayDoc.totalTime || 0) === 0) {
      logger.info("Zero-time detected, skipping reduction", {
        challengeId,
        userId,
        prevDate: prevDoc.date,
        prevTotal: prevDoc.totalTime,
        currentTotal: todayDoc.totalTime,
      });
      return res
        .status(200)
        .json({ message: "Skipped day with zero screen time" });
    }
    ...
    const gapDays = Math.max(
      ...
      Math.round(
        (startOfCurrentDay -
          new Date(new Date(prevDoc.date).setHours(0, 0, 0, 0))) /
          dayMs,
      ),
    );
    const deltaHours = Math.max(
      0,
      (prevDoc.totalTime - todayDoc.totalTime) / 3600,
    );
    ...
    if (
      now.getTime() - participant.lastUpdate >= 3600000 ||
      manualTrigger ||
      lastUpdateDay.getTime() !== startOfCurrentDay.getTime()
    ) {
      const updatedChallenge = await Challenge.findOneAndUpdate(
        ...
        {
          $set: {
            "participants.$.reduction": newReduction,
            "participants.$.lastUpdate": now.getTime(),
          },
        },
        { new: true, runValidators: true },
      );
      if (!updatedChallenge) throw new Error("Failed to update participant");
      logger.info("Progress updated (uncapped, gap-aware)", {
        challengeId,
        userId,
        addedHours,
        gapDays,
      ...
      logger.info("Progress update skipped, within 1-hour interval", {
        challengeId,
        userId,
        lastUpdate: participant.lastUpdate,
        now: now.getTime(),
...
router.get("/leaderboard", authMiddleware, async (req, res) => {
  try {
    ...
    logger.info("Leaderboard participant userIds:", { allUserIds });
    ...
    try {
      ...
      for (const challenge of challenges) {
        ...
        for (const participant of challenge.participants) {
          ...
          let prevDoc = await ScreenTime.findOne({
            userId,
            date: { $lt: currentDay, $gte: anchorYmd },
          }).sort({ date: -1 });
          if (!prevDoc) {
            prevDoc = await ScreenTime.findOne({
              userId,
          ...
          if (!prevDoc || !todayDoc) {
            logger.info(
              "Insufficient data (cron), need current day and a previous tracked day since anchor",
              {
                challengeId,
                userId,
                hasPrev: !!prevDoc,
          ...
          if (
            (prevDoc.totalTime || 0) === 0 ||
            (todayDoc.totalTime || 0) === 0
          ) {
            logger.info("Zero-time detected (cron), skipping reduction", {
              challengeId,
              userId,
              prevDate: prevDoc.date,
              prevTotal: prevDoc.totalTime,
          ...
          const gapDays = Math.max(
            1,
            Math.round((new Date(currentDay) - new Date(prevDoc.date)) / dayMs),
          );
          const deltaHours = Math.max(
            0,
            (prevDoc.totalTime - todayDoc.totalTime) / 3600,
      ...
      logger.error("Error in daily progress update", {
        error: err.message,
        stack: err.stack,
      });
    }
